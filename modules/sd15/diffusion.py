...
class UNET_AttentionBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor, context: torch.Tensor):
        ...
        x = x.view((n, c, h * w))  # B Features H*W
        x = x.transpose(-1, -2)  # B H*W Features
        ...
        x = x.transpose(-1, -2)  # B Features H*W
        x = x.view((n, c, h, w))  # B Features H W
...
class Upsample(nn.Module):
    ...
    def forward(self, x):
        # x: B Features H W
        x = F.interpolate(x, scale_factor=2, mode='nearest')  # B Features H*2 W*2
...
class UNET(nn.Module):
    def __init__(self):
        ...
        self.decoders = nn.ModuleList([
            SwitchSequential(UNET_ResidualBlock(2560, 1280)),  # B 2560 H/64 W/64
...
class Diffusion(nn.Module):
    ...
    def forward(self, latent: torch.Tensor, context: torch.Tensor, time: torch.Tensor):
        ...
        output = self.unet(latent, context, time)  # B 320 H/8 W/8
        output = self.final(output)  # B 4 H/8 W/8
