...
class DDPMSampler:
    ...
    def _get_variance(self, timestep: int, device=None, dtype=None) -> torch.Tensor:
        prev_t = self._get_previous_timestep(timestep)
        alpha_cumprod = self.alpha_cumprod
        one = self.one
        if device is not None or dtype is not None:
            alpha_cumprod = alpha_cumprod.to(device=device, dtype=dtype)
            one = one.to(device=device, dtype=dtype)
        
        alpha_prod_t = alpha_cumprod[timestep]
        alpha_prod_t_prev = alpha_cumprod[prev_t] if prev_t >= 0 else one
        current_beta_t = 1 - alpha_prod_t / alpha_prod_t_prev
        
        # ddpm paper formula 7
    ...
    def step(self, timestep: int, latents: torch.Tensor, model_output: torch.Tensor, try_use_cached_noise: bool = True) -> torch.Tensor:
        ...
        alpha_cumprod = self.alpha_cumprod.to(device=device, dtype=dtype)
    ...
    def add_noise(self, original_samples: torch.FloatTensor, timesteps: torch.IntTensor) -> torch.FloatTensor:
        ...
        sqrt_one_minus_alpha_prod = (1 - alpha_cumprod[timesteps]) ** 0.5  # std
